...
Endpoints
  ...
  GET  /health    → {"status": "ok", "gee_initialized": bool}
...
# ── GEE initialisation ────────────────────────────────────────────────────────
_key_env = os.environ.get("GEE_SERVICE_ACCOUNT_KEY")
if _key_env:
    try:
        _key_data = json.loads(_key_env)
        _credentials = ee.ServiceAccountCredentials(
            email=_key_data["client_email"],
            key_data=json.dumps(_key_data),
        )
        ee.Initialize(credentials=_credentials, project=_key_data.get("project_id", "ggr437-1007998482"))
...
def _sample_points_in_radius(center_lat: float, center_lon: float, radius_km: float, n: int) -> list[tuple[float, float]]:
    """Sample n points: point 0 = center, 1..n-1 on a circle at radius_km (lat-adjusted)."""
    ...
    for i in range(1, n):
        ...
        lon = center_lon + (radius_deg * math.sin(angle)) / max(0.01, math.cos(math.radians(center_lat)))
...
def get_indices(
    ...
    if not _GEE_INITIALIZED:
        raise RuntimeError(
            ...
            "Set GEE_SERVICE_ACCOUNT_KEY (JSON) in the environment."
        )
...
def indices():
    ...
    # Area mode: sample n_points in radius_km, aggregate indices (mean for most, max for MNDWI/dMNDWI)
    if all(k in body for k in ("center_lat", "center_lon", "radius_km", "n_points")):
        try:
            center_lat = float(body["center_lat"])
            center_lon = float(body["center_lon"])
            radius_km = float(body["radius_km"])
            n_points = int(body["n_points"])
            start = str(body.get("start", body.get("post_start", "")))
            end = str(body.get("end", body.get("post_end", "")))
        except (ValueError, TypeError, KeyError) as exc:
            return jsonify({"error": f"Area mode: invalid parameter: {exc}"}), 400
        if not start or not end:
            return jsonify({"error": "Area mode: provide start and end (or post_start, post_end)"}), 400
        ...
        points = _sample_points_in_radius(center_lat, center_lon, radius_km, n_points)
        results = []
        for lat, lon in points:
            try:
                r = get_indices(lat, lon, start, end, buffer_m, years_back, baseline_start, baseline_end)
                results.append(r)
            except Exception as exc:
                return jsonify({"error": str(exc), "at_point": [lat, lon]}), 500
        def _mean(key):
        ...
        return jsonify({
            ...
            "post_start": start,
            "post_end": end,
            "baseline_start": first.get("baseline_start"),
            "baseline_end": first.get("baseline_end"),
            "baseline_years": first.get("baseline_years"),
            "baseline_type": first.get("baseline_type"),
            "nbr": _mean("nbr"), "ndvi": _mean("ndvi"), "mndwi": _max("mndwi"), "bsi": _mean("bsi"),
            "nbr_pre": _mean("nbr_pre"), "ndvi_pre": _mean("ndvi_pre"),
            "bsi_pre": _mean("bsi_pre"), "mndwi_pre": _mean("mndwi_pre"),
            "dnbr": _mean("dnbr"), "dndvi": _mean("dndvi"), "dbsi": _mean("dbsi"), "dmndwi": _max("dmndwi"),
            "land_class": first.get("land_class"), "nbr_label": first.get("nbr_label"),
            "mndwi_label": first.get("mndwi_label"), "dndvi_label": first.get("dndvi_label"),
            "dbsi_label": first.get("dbsi_label"),
        })
...
def health():
    return jsonify({"status": "ok", "gee_initialized": _GEE_INITIALIZED})
